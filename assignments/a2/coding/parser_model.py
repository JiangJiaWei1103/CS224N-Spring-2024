...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.emb_to_hidden_weight = nn.Parameter(torch.empty(in_features, hidden_size))
    ...
    #     This is the common paradigm to init params.
    #     """
    #     # Embedding to hidden
    #     nn.init.xavier_uniform_(self.emb_to_hidden_weight)
    #     nn.init.uniform_(self.emb_to_hidden_bias)
    ...
    def embedding_lookup(self, w):
        ...
        def _batched_index_select(e: torch.Tensor, w: torch.Tensor) -> torch.Tensor:
            """Batched index select.
            
            See https://discuss.pytorch.org/t/batched-index-select/9115/7 .
            """
            e = e.unsqueeze(dim=0).expand(bs, -1, -1)  # Expand batch dim
            w = w.unsqueeze(dim=2).expand(-1, -1, self.embed_size)  # Expand embedding dim
            x = torch.gather(e, dim=1, index=w)

            return x

        bs, m = w.shape

        x = torch.index_select(self.embeddings, dim=0, index=w.reshape(-1))  # (bs * m, d)
        # or x = _batched_index_select(self.embeddings, w)  # (b, m, d)
        x = x.reshape(bs, -1)
    ...
    def forward(self, w):
        ...
        x = self.embedding_lookup(w)  # (b, m * d)
        h = F.relu(x @ self.emb_to_hidden_weight + self.emb_to_hidden_bias)  # (b, h)
        h = self.dropout(h)
        logits = h @ self.hidden_to_logits_weight + self.hidden_to_logits_bias  # (b, c)
