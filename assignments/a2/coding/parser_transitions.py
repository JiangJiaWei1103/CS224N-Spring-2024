...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        assert transition in {"S", "LA", "RA"}, "Invalid transition is passed."
...
def minibatch_parse(sentences, model, batch_size):
    ...
    def _is_terminated(pp: PartialParse) -> bool:
        """Determine if the terminal state is reached."""
    ...
    while len(unfinished_parses) > 0:
        ...
        # Predict next transitions of the current batch
        transitions = model.predict(minibatch)

        # Take one parse step and check if finished
