...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        # Initialize the decoder's first hidden and cell states
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        # Attention key projection (see eq(7) in pdf)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = (enc_hiddens_proj @ dec_hidden.unsqueeze(dim=-1)).squeeze(dim=-1)  # (b, src_len)
        ...
        a_t = (alpha_t.unsqueeze(dim=1) @ enc_hiddens).squeeze(dim=1)   # (b, h*2)

        # Construct combined-output vector 
